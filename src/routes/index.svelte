<script lang="ts">
	...
	const goToSearch = (e) => {
		...
		e.preventDefault();
		goto(`/search?university=${universityData}&course=${currentCourse}&major=${currentMajor}`);
	};
</script>

<h1 class="text-4xl text-center font-bold text-slate-400 dark:text-slate-100 my-8">
	WELCOME TO <span
...
<form class="mb-[40vh]" on:submit={goToSearch}>
	<div class="wrapper flex flex-col items-center justify-center">
		...
		<div class="university-label-wrapper flex justify-start items-center w-[55%] my-5">
			<label for="university" class="text-center text-3xl font-semibold text-black dark:text-white"
		...
		>
			<div class="dropdown z-80 cursor-pointer border-r-4 border-blue-100 rounded-sm relative">
				<div
					class="chosen-option flex items-center justify-around p-2"
					on:click={() => (displayCountryDropdown = !displayCountryDropdown)}
				>
					<input
			...
			<div class="university_search_field flex items-center justify-center relative w-full">
				<input
					type="text"
		...
		<div class="course-label-wrapper flex justify-start items-center w-[55%] my-5">
			<label for="course" class="text-center text-3xl font-semibold text-black dark:text-white"
				>Course</label
			>
		</div>
		...
		<div
			...
			style="box-shadow: 0 8px 6px -10px #b3c6ff;"
		>
			<div class="course_search_field z-60 flex items-center justify-center w-full relative">
				<input
					type="text"
						...
						>
							{#each Object.entries(courseDataSearchResult) as [key, value]}
								<li
									class="hover:bg-indigo-500 dark:hover:bg-cyan-400 hover:text-white py-2 pl-9 text-lg"
									on:click={() => {
										currentCourse = value.code + ' - ' + value.title;
										displayCourseDropdown = false;
									}}
								>
		...
		<div class="major-label-wrapper flex justify-start items-center w-[55%] my-5">
			<label for="major" class="text-center text-3xl font-semibold text-black dark:text-white"
				>Major/Minor</label
			>
		</div>
		<div
			...
			style="box-shadow: 0 8px 6px -10px #b3c6ff;"
		>
			<div class="major_search_field flex items-center justify-center w-full relative">
				<input
					type="text"
					class="input w-full text-xl border-none"
					placeholder="Search"
						...
						>
							{#each Object.entries(majorDataSearchResult) as [key, value]}
								<li
									class="hover:bg-indigo-500 dark:hover:bg-cyan-400 hover:text-white py-2 pl-9 text-lg"
									on:click={() => {
										currentMajor = value.code + ' - ' + value.title;
